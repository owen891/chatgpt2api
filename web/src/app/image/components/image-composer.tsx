...
export function ImageComposer({
  ...
  const modelOptions = useMemo(
    () => imageModels.map((model) => ({
      ...
      isPinnedUpstream: Boolean(model.image_upstream),
    })),
    [imageModels],
  );
              ...
              <div className="flex items-end justify-between gap-2 sm:gap-3">
                <div className="hide-scrollbar flex min-w-0 flex-1 flex-nowrap items-center gap-1.5 overflow-x-auto pb-0.5 sm:flex-wrap sm:gap-3 sm:overflow-visible sm:pb-0">
                  ...
                  <Select value={imageModel} onValueChange={(value) => onImageModelChange(value as ImageModel)}>
                    <SelectTrigger className="h-9 w-[130px] shrink-0 rounded-full border-0 bg-stone-100 px-3 text-xs font-semibold text-stone-900 shadow-none sm:h-10 sm:w-[180px] sm:px-4 sm:text-sm">
                      <div className="flex min-w-0 items-center gap-2">
                        <img src="/openai.svg" alt="" aria-hidden="true" className="size-3.5 shrink-0 sm:size-4" />
                    ...
                    <SelectContent className="z-[120]">
                      {modelOptions.map((option) => (
                        ...
                        >
                          ...
                          {option.isPinnedUpstream ? <span className="ml-2 text-[10px] font-normal text-stone-500">固定渠道</span> : null}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
