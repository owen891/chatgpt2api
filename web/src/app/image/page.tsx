...
function filterImageModels(items: Model[]): Model[] {
  return items
    .filter((item, index, list) => {
      const id = String(item.id || "").trim();
      const root = String(item.root || "").trim();
      return (id.toLowerCase().includes("image") || root.toLowerCase().includes("image"))
        && list.findIndex((candidate) => candidate.id === id) === index;
    });
}

function normalizeStoredImageModel(value: string | null, availableModels: Model[]): ImageModel {
  const normalized = String(value || "").trim();
  if (normalized && availableModels.some((model) => model.id === normalized)) {
    return normalized;
  }
  return availableModels.find((model) => model.id === "gpt-image-2")?.id || availableModels[0]?.id || "gpt-image-2";
}
...
function ImagePageContent() {
    ...
    async (conversationId: string) => {
        ...
        await updateConversation(conversationId, (current) => {
          ...
          const turns = conversation.turns.map((turn) => {
            ...
            const images = turn.images.map((image) => {
              ...
              if (cancelledTaskIdsRef.current.has(taskId) && task?.status !== "cancelled") {
                return image;
              }
      ...
      try {
        ...
        const pendingImages = activeTurn.images.filter(
          (image) => image.status === "loading" && !cancelledTaskIdsRef.current.has(image.taskId || image.id),
        );
        ...
        const cancelledSubmittedIds = submitted
          .map((task) => task.id)
          .filter((taskId) => cancelledTaskIdsRef.current.has(taskId));
        if (cancelledSubmittedIds.length > 0) {
          await cancelImageTasks(cancelledSubmittedIds);
        }
        ...
        while (true) {
          ...
          const loadingTaskIds =
            latestTurn?.images.flatMap((image) =>
              image.status === "loading" && image.taskId && !cancelledTaskIdsRef.current.has(image.taskId)
                ? [image.taskId]
                : [],
            ) || [];
  ...
  const cancelPendingImages = useCallback(
    async (conversationId: string, turnId: string, taskIds: string[]) => {
      ...
      normalizedTaskIds.forEach((taskId) => cancelledTaskIdsRef.current.add(taskId));
      ...
      await updateConversation(conversationId, (current) => {
        ...
        return {
          ...
          turns: conversation.turns.map((turn) => {
            if (turn.id !== turnId) return turn;
            const images = turn.images.map((image) =>
              image.status === "loading" && taskIdSet.has(image.taskId || image.id)
                ? {
                    ...image,
      ...
      try {
        const result = await cancelImageTasks(normalizedTaskIds);
        const taskMap = new Map(result.items.map((task) => [task.id, task]));
          ...
          return {
            ...
            turns: conversation.turns.map((turn) => {
              ...
              const images = turn.images.map((image) => {
                ...
                return task ? taskDataToStoredImage(image, task) : image;
              });
              return { ...turn, ...deriveTurnStatus({ ...turn, images }), images };
            }),
          };
        });
        toast.success(normalizedTaskIds.length === 1 ? "已停止这张图片" : "已停止剩余图片");
      } catch (error) {
  ...
  const handleCancelImage = useCallback(
    async (conversationId: string, turnId: string, imageId: string) => {
      const conversation = conversationsRef.current.find((item) => item.id === conversationId);
      const image = conversation?.turns.find((turn) => turn.id === turnId)?.images.find((item) => item.id === imageId);
      if (!image || image.status !== "loading") return;
      await cancelPendingImages(conversationId, turnId, [image.taskId || image.id]);
    },
    [cancelPendingImages],
  );
