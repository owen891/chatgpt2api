...
import {
  ...
  type ImageUpstreamChannel,
  type ImageUpstreamsSettings,
...
const DEFAULT_IMAGE_UPSTREAMS: ImageUpstreamsSettings = {
  max_attempts: 2,
  alert_webhook_url: "",
  channels: [],
};

function normalizeImageUpstreams(value: unknown): ImageUpstreamsSettings {
  const source = typeof value === "object" && value !== null ? value as Partial<ImageUpstreamsSettings> : {};
  const channels = Array.isArray(source.channels) ? source.channels : [];
  return {
    max_attempts: Math.min(10, Math.max(1, Number(source.max_attempts) || 2)),
    alert_webhook_url: String(source.alert_webhook_url || ""),
    channels: channels.map((item, index) => {
      const channel = item as Partial<ImageUpstreamChannel>;
      return {
        id: String(channel.id || `image-upstream-${index + 1}`),
        name: String(channel.name || `生图上游 ${index + 1}`),
        model_alias: String(channel.model_alias || "").toLowerCase(),
        enabled: channel.enabled !== false,
        priority: Math.max(0, Number(channel.priority) || (index + 1) * 10),
        default: Boolean(channel.default),
        base_url: String(channel.base_url || ""),
        api_key: "",
        has_api_key: Boolean(channel.has_api_key),
        clear_api_key: false,
        timeout_secs: Math.max(1, Number(channel.timeout_secs) || 360),
        proxy_url: String(channel.proxy_url || ""),
        supports_generation: channel.supports_generation !== false,
        supports_edits: channel.supports_edits !== false,
        failure_threshold: Math.max(1, Number(channel.failure_threshold) || 3),
        cooldown_secs: Math.max(1, Number(channel.cooldown_secs) || 120),
        max_concurrency: Math.min(100, Math.max(1, Number(channel.max_concurrency) || 3)),
        requests_per_minute: Math.min(10000, Math.max(1, Number(channel.requests_per_minute) || 60)),
        model_mappings: Array.isArray(channel.model_mappings)
          ? channel.model_mappings.map((mapping) => ({
            client_model: String(mapping.client_model || ""),
...
function normalizeConfig(config: SettingsConfig): SettingsConfig {
  ...
  return {
    ...
    image_upstreams: normalizeImageUpstreams(config.image_upstreams),
    backup: {
      ...backup,
  ...
  saveConfig: async () => {
    ...
    try {
      const data = await updateSettingsConfig({
        ...
        image_upstreams: {
          max_attempts: Math.min(10, Math.max(1, Number(config.image_upstreams?.max_attempts) || 2)),
          alert_webhook_url: String(config.image_upstreams?.alert_webhook_url || "").trim(),
          channels: (config.image_upstreams?.channels || []).map((channel) => ({
            ...channel,
            name: String(channel.name || "").trim(),
            model_alias: String(channel.model_alias || "").trim().toLowerCase(),
            base_url: String(channel.base_url || "").trim().replace(/\/+$/, ""),
            api_key: String(channel.api_key || "").trim(),
            proxy_url: String(channel.proxy_url || "").trim(),
            priority: Math.max(0, Number(channel.priority) || 0),
            timeout_secs: Math.min(600, Math.max(1, Number(channel.timeout_secs) || 360)),
            failure_threshold: Math.min(20, Math.max(1, Number(channel.failure_threshold) || 3)),
            cooldown_secs: Math.min(3600, Math.max(1, Number(channel.cooldown_secs) || 120)),
            max_concurrency: Math.min(100, Math.max(1, Number(channel.max_concurrency) || 3)),
            requests_per_minute: Math.min(10000, Math.max(1, Number(channel.requests_per_minute) || 60)),
            model_mappings: (channel.model_mappings || []).map((mapping) => ({
              client_model: String(mapping.client_model || "").trim(),
              upstream_model: String(mapping.upstream_model || "").trim(),
            })).filter((mapping) => mapping.client_model && mapping.upstream_model),
          })),
        },
        backup: {
          ...(config.backup as BackupSettings),
