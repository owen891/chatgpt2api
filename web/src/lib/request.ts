...
request.interceptors.response.use(
    ...
    async (error: AxiosError<ErrorPayload>) => {
        ...
        const shouldRedirect = originalConfig?.redirectOnUnauthorized !== false;
        const method = String(originalConfig?.method || "GET").toUpperCase();
        const canRetry = ["GET", "HEAD", "OPTIONS"].includes(method);
        if (status === 401 && shouldRedirect && canRetry && !originalConfig?.authRetry && typeof window !== "undefined" && !window.location.pathname.startsWith("/login")) {
            const authKey = await getStoredAuthKey();
            if (authKey) {
                try {
                    ...
                    return request.request({...originalConfig, authRetry: true} as RequestConfig);
                } catch {
                    // 认证失败后继续走统一跳转流程。
                }
            }
