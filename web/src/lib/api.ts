...
export type ImageUpstreamMapping = {
  client_model: string;
  upstream_model: string;
};

export type ImageUpstreamChannel = {
  id: string;
  name: string;
  model_alias: string;
  enabled: boolean;
  ...
  cooldown_secs: number | string;
  max_concurrency: number | string;
  requests_per_minute: number | string;
  model_mappings: ImageUpstreamMapping[];
};

export type ImageUpstreamsSettings = {
  max_attempts: number | string;
  alert_webhook_url: string;
  channels: ImageUpstreamChannel[];
};

export type ImageUpstreamTestResult = {
  ok: boolean;
  status: number;
  latency_ms: number;
  error: string | null;
...
export type ImageUpstreamRuntimeStatus = {
  circuit_open: boolean;
  cooldown_remaining_secs: number;
  failure_count: number;
  last_error: string;
  last_test: ImageUpstreamTestResult | null;
...
export type SettingsConfig = {
  ...
  image_upstreams?: ImageUpstreamsSettings;
...
export async function cancelImageTasks(taskIds: string[]) {
  return httpRequest<ImageTaskListResponse>("/api/image-tasks/cancel", {
    method: "POST",
...
export async function testImageUpstream(channel: ImageUpstreamChannel) {
  return httpRequest<{ result: ImageUpstreamTestResult }>("/api/image-upstreams/test", {
    method: "POST",
    body: { channel },
  });
}

export async function fetchImageUpstreamModels(channel: ImageUpstreamChannel) {
  return httpRequest<{ result: ImageUpstreamTestResult }>("/api/image-upstreams/models", {
    method: "POST",
    body: { channel },
  });
}

export async function fetchImageUpstreamStatus() {
  return httpRequest<{ channels: Record<string, ImageUpstreamRuntimeStatus> }>("/api/image-upstreams/status");
}
...
export type RegisterProvider = {
  ...
  mailboxes_stats?: Record<string, number | undefined>;
  mailboxes_parse_stats?: Record<string, unknown>;
...
export type RegisterConfig = {
  ...
  expected_quota_per_account: number;
  ...
  history?: Array<{ id: string; mode: string; status: string; reason?: string; started_at: string; finished_at: string; start_quota?: number; end_quota?: number; start_available?: number; end_available?: number; success?: number; fail?: number }>;
};
...
export async function updateRegisterConfig(config: Partial<RegisterConfig>) {
  return httpRequest<{ register: RegisterConfig }>("/api/register", { method: "POST", body: config });
}

export async function startRegister() {
  return httpRequest<{ register: RegisterConfig }>("/api/register/start", { method: "POST", body: {} });
}

export async function stopRegister() {
  return httpRequest<{ register: RegisterConfig }>("/api/register/stop", { method: "POST", body: {} });
}

export async function resetRegister() {
  return httpRequest<{ register: RegisterConfig }>("/api/register/reset", { method: "POST", body: {} });
}
...
export async function fetchGptMailStatus(provider: RegisterProvider, force = true) {
  return httpRequest<{ status: RegisterGptMailStatus }>("/api/register/gptmail/status", {
    method: "POST",
    body: { provider, force },
  });
}

export async function resetOutlookPool(scope: "all" | "retryable" | "invalid" | "unused" | "failed" = "all") {
