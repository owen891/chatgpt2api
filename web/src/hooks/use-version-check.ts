...
const latestVersionUrl = webConfig.repositoryRawBaseUrl ? `${webConfig.repositoryRawBaseUrl}/VERSION` : "";
const latestChangelogUrl = webConfig.repositoryRawBaseUrl ? `${webConfig.repositoryRawBaseUrl}/CHANGELOG.md` : "";
...
export function useVersionCheck() {
  ...
  const checkLatestRelease = useCallback(
    async (showMessage = false) => {
      setChecking(true);
      try {
        if (!latestVersionUrl || !latestChangelogUrl) throw new Error();
