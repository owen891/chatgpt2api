...
class ImageTasksApiTests(unittest.TestCase):
    ...
    def test_cancel_tasks(self):
        ...
        self.assertEqual(response.status_code, 200, response.text)
        self.assertEqual([item["status"] for item in response.json()["items"]], ["cancelled", "cancelled"])
        self.assertEqual(self.fake_service.cancel_calls[0][1], ["task-1", "task-2"])

    def test_cancel_tasks_requires_authentication(self):
        response = self.client.post("/api/image-tasks/cancel", json={"task_ids": ["task-1"]})
