...
def _ensure_public_image_url(url: str) -> None:
    ...
    for address in addresses:
        try:
            ...
            raise HTTPException(status_code=400, detail={"error": "image_url host returned an invalid address"}) from exc
        if not ip.is_global:
...
def _download_image_url(url: str) -> ImageInput:
    ...
    request_url = source
    response = None
    for _ in range(MAX_IMAGE_REFERENCE_REDIRECTS + 1):
        _ensure_public_image_url(request_url)
        try:
