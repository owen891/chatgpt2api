...
class ImageTaskCancelled(RuntimeError):
    """协作式停止信号：底层请求返回前，任务状态也必须保持为已停止。"""

    cancel_image_task = True
...
class ImageTaskService:
    ...
    def cancel_tasks(self, identity: dict[str, object], task_ids: list[str]) -> dict[str, Any]:
        """停止当前用户仍在排队或运行中的图片任务。"""
        owner = _owner_id(identity)
        requested_ids = list(dict.fromkeys(_clean(task_id) for task_id in task_ids if _clean(task_id)))
        with self._lock:
            ...
            for task_id in requested_ids:
                ...
                if task.get("status") in UNFINISHED_STATUSES:
                    started = task.get("started_ts") or task.get("created_ts") or time.time()
                    task.update(
                        {
                            ...
                            "error": "用户已停止该图片任务",
                            "data": [],
                            "duration_ms": max(0, int((time.time() - float(started)) * 1000)),
                        }
                    )
    ...
    def _run_task(
        ...
        def progress_callback(step: str) -> None:
            if self._is_cancelled(key):
                raise ImageTaskCancelled("用户已停止该图片任务")
        ...
        try:
            ...
            self._log_call(
                ...
                image_upstream_attempts=result.get("_image_upstream_attempts"),
                image_upstream_selected=_clean(result.get("_image_upstream_selected")),
            )
    ...
    def _log_call(
        ...
        *,
        ...
        account_email: str = "",
        image_upstream_attempts: object = None,
        image_upstream_selected: str = "",
    ) -> None:
    ...
    def _run_resume_poll(
        ...
        try:
            ...
            if not self._update_task(key, status=TASK_STATUS_SUCCESS, data=data, error="", duration_ms=int((time.time() - started) * 1000)):
                return
            self._log_call(
                identity,
                mode,
            ...
            duration_ms = int((time.time() - started) * 1000)
            if not self._update_task(key, status=TASK_STATUS_ERROR, error=error_message, data=[], duration_ms=duration_ms):
                return
            self._log_call(
                identity,
                mode,
