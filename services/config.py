...
IMAGE_UPSTREAM_RUNTIME_STATE_FILE = DATA_DIR / "image_upstream_runtime.json"
...
def _normalize_image_upstreams(value: object) -> dict[str, object]:
    source = value if isinstance(value, dict) else {}
    channels_source = source.get("channels") if isinstance(source.get("channels"), list) else []
    channels: list[dict[str, object]] = []
    seen_ids: set[str] = set()
    has_default = False
    for index, item in enumerate(channels_source):
        if not isinstance(item, dict):
            continue
        ...
        model_alias = str(item.get("model_alias") or "").strip().lower()
        mappings_source = item.get("model_mappings") if isinstance(item.get("model_mappings"), list) else []
        ...
        for mapping in mappings_source:
            ...
            upstream_model = str(mapping.get("upstream_model") or "").strip()
            if not client_model or not upstream_model or client_model in mapped_models:
                continue
            mapped_models.add(client_model)
            mappings.append({"client_model": client_model, "upstream_model": upstream_model})
        is_default = _normalize_bool(item.get("default"), False) and not has_default
        has_default = has_default or is_default
        channels.append({
            "id": channel_id,
            "name": str(item.get("name") or f"Image upstream {index + 1}").strip() or f"Image upstream {index + 1}",
            "model_alias": model_alias,
            "enabled": _normalize_bool(item.get("enabled"), True),
            "priority": _normalize_positive_int(item.get("priority"), (index + 1) * 10, 0),
            "default": is_default,
            ...
            "failure_threshold": min(20, _normalize_positive_int(item.get("failure_threshold"), 3, 1)),
            "cooldown_secs": min(3600, _normalize_positive_int(item.get("cooldown_secs"), 120, 1)),
            "max_concurrency": min(100, _normalize_positive_int(item.get("max_concurrency"), 3, 1)),
            "requests_per_minute": min(10000, _normalize_positive_int(item.get("requests_per_minute"), 60, 1)),
            "model_mappings": mappings,
        })
    return {
        "max_attempts": min(10, _normalize_positive_int(source.get("max_attempts"), 2, 1)),
        "alert_webhook_url": str(source.get("alert_webhook_url") or "").strip(),
        "channels": channels,
    }


def _validate_image_upstreams(settings: dict[str, object]) -> None:
    aliases: set[str] = set()
    selectable_model_ids: set[str] = set()
    client_models = {
        str(mapping.get("client_model") or "").strip()
        for channel in settings.get("channels") or []
        if isinstance(channel, dict)
        for mapping in channel.get("model_mappings") or []
        if isinstance(mapping, dict)
    }
    alert_webhook_url = str(settings.get("alert_webhook_url") or "")
    if alert_webhook_url:
        parsed = urlparse(alert_webhook_url)
        if parsed.scheme not in {"http", "https"} or not parsed.netloc:
            raise ValueError("生图上游告警 Webhook 必须是 http 或 https URL")
    for channel in settings.get("channels") or []:
        if not isinstance(channel, dict):
            continue
        alias = str(channel.get("model_alias") or "")
        if alias:
            if not re.fullmatch(r"[a-z0-9][a-z0-9_-]{1,63}", alias):
                raise ValueError(f"生图上游 {channel.get('name') or channel.get('id')} 的模型别名只能包含小写字母、数字、连字符和下划线")
            if alias in aliases:
                raise ValueError(f"生图上游模型别名重复：{alias}")
            if alias in client_models:
                raise ValueError(f"生图上游模型别名不能与客户端模型名冲突：{alias}")
            aliases.add(alias)
        ...
        if not base_url or parsed.scheme not in {"http", "https"} or not parsed.netloc:
            raise ValueError(f"生图上游 {channel.get('name') or channel.get('id')} 的 Base URL 必须是 http 或 https URL")
        if not channel.get("model_mappings"):
            raise ValueError(f"生图上游 {channel.get('name') or channel.get('id')} 至少需要一个模型映射")
        mappings = [item for item in channel.get("model_mappings") or [] if isinstance(item, dict)]
        if alias:
            for mapping in mappings:
                client_model = str(mapping.get("client_model") or "").strip()
                selectable_id = alias if len(mappings) == 1 else f"{alias}--{client_model}"
                if selectable_id in selectable_model_ids:
                    raise ValueError(f"生图上游固定渠道模型 ID 重复：{selectable_id}")
                selectable_model_ids.add(selectable_id)
...
class ConfigStore:
    ...
    def update(self, data: dict[str, object]) -> dict[str, object]:
        ...
        if "image_upstreams" in next_data:
            incoming_upstreams = next_data.get("image_upstreams")
            if isinstance(incoming_upstreams, dict):
                existing_channels = {
                    str(channel.get("id") or ""): channel
                    for channel in self.get_image_upstreams_settings().get("channels", [])
                    if isinstance(channel, dict)
                }
                incoming_channels = incoming_upstreams.get("channels")
                if isinstance(incoming_channels, list):
                    incoming_upstreams = dict(incoming_upstreams)
                    retained_channels: list[object] = []
                    for channel in incoming_channels:
                        ...
                        if item.get("clear_api_key"):
                            ...
                            item["api_key"] = str(previous.get("api_key") or "")
                        retained_channels.append(item)
                    incoming_upstreams["channels"] = retained_channels
            next_data["image_upstreams"] = _normalize_image_upstreams(incoming_upstreams)
            _validate_image_upstreams(next_data["image_upstreams"])
    ...
    def get_image_upstreams_settings(self) -> dict[str, object]:
        return _normalize_image_upstreams(self.data.get("image_upstreams"))

    def get_image_upstream_runtime_state(self) -> dict[str, object]:
        return _read_json_object(IMAGE_UPSTREAM_RUNTIME_STATE_FILE, name="image upstream runtime state")

    def save_image_upstream_runtime_state(self, state: dict[str, object]) -> None:
        DATA_DIR.mkdir(parents=True, exist_ok=True)
        IMAGE_UPSTREAM_RUNTIME_STATE_FILE.write_text(
            json.dumps(state, ensure_ascii=False, indent=2) + "\n",
            encoding="utf-8",
        )

    def get_public_image_upstreams_settings(self) -> dict[str, object]:
        settings = copy.deepcopy(self.get_image_upstreams_settings())
        ...
        if isinstance(channels, list):
            for channel in channels:
                ...
                channel.pop("api_key", None)
                channel["has_api_key"] = bool(api_key)
        return settings
