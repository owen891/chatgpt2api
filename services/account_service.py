...
class ImageAccountSelectionError(RuntimeError):
    DEFAULTS: dict[str, tuple[int, str, str]] = {
        ...
        "auth_invalid": (401, "authentication_error", "authentication_error"),
    }

    def __init__(self, kind: str, message: str = "") -> None:
        self.kind = kind if kind in self.DEFAULTS else "unavailable"
        self.status_code, self.error_type, self.code = self.DEFAULTS[self.kind]
...
class AccountService:
    ...
    def _is_account_selectable(account: dict, *, allow_limited: bool) -> bool:
        if not isinstance(account, dict):
            return False
        statuses = {"正常", "限流"} if allow_limited else {"正常"}
        if account.get("status") not in statuses:
            return False
    ...
    def _is_image_account_available(cls, account: dict) -> bool:
        ...
        if bool(account.get("image_quota_unknown")):
            return True
        # 本地 quota 只是缓存；只有远端确认的“限流”才应把账号排除。
    ...
    @classmethod
    def _extract_image_quota_from_limits(cls, limits: object) -> tuple[int | None, str | None, bool | None]:
        if not isinstance(limits, list) or not limits:
            return None, None, None
        for item in limits:
            if not isinstance(item, dict):
                continue
            feature = str(item.get("feature_name") or item.get("feature") or item.get("name") or "").strip().lower()
            if feature not in {"image_gen", "image_generation", "image", "images"}:
                continue
            try:
                quota = max(0, int(float(str(item.get("remaining") or 0))))
            except (TypeError, ValueError):
                quota = 0
            restore_at = str(item.get("reset_after") or item.get("restore_at") or item.get("reset_at") or "").strip() or None
    ...
    def _normalize_account(self, item: dict) -> dict | None:
        ...
        has_quota_state = self._has_value(normalized.get("image_quota_unknown"))
        if derived_unknown is not None and not has_quota_state:
            normalized["image_quota_unknown"] = derived_unknown
        elif not has_quota_state:
            ...
            while True:
                if not self._list_ready_candidate_tokens(excluded_tokens, plan_type, source_type, plan_types):
                    ...
                    matched = [
                        ...
                        if (item.get("access_token") or "") not in excluded
                        and self._account_matches_plan_type(item, plan_type)
                        and self._account_matches_any_plan_type(item, plan_types)
                        and self._account_matches_source_type(item, source_type)
                    ]
                    if matched and all(str(item.get("status") or "") == "限流" for item in matched):
                        raise ImageAccountSelectionError("quota_exhausted", "all matched accounts are rate limited")
                    raise ImageAccountSelectionError("unavailable", "no image account is ready")
    ...
    def get_available_access_token(
        ...
        """
        max_attempts = 3
        external_excluded = set(excluded_tokens or set())
        attempted_tokens: set[str] = set()
        saw_quota_exhausted = False
        saw_unavailable = False
        saw_auth_invalid = False
        for _attempt in range(max_attempts):
            try:
                access_token = self._acquire_next_candidate_token(
                    excluded_tokens=external_excluded | attempted_tokens,
                    plan_type=plan_type,
            ...
            try:
                ...
                if name in {"InvalidAccessTokenError", "TerminalRefreshTokenError"} or any(
                    marker in message for marker in ("invalid access token", "invalid_grant", "token_invalidated")
                ):
                    saw_auth_invalid = True
                else:
            ...
            if str((account or {}).get("status") or "") == "限流":
                ...
                saw_unavailable = True
            self.release_image_slot(access_token)
        if saw_auth_invalid and not saw_unavailable and not saw_quota_exhausted:
            raise ImageAccountSelectionError("auth_invalid", f"authentication failed for {len(attempted_tokens)} accounts")
        if saw_quota_exhausted and not saw_unavailable and not saw_auth_invalid:
            raise ImageAccountSelectionError("quota_exhausted", f"quota exhausted after {len(attempted_tokens)} accounts")
        raise ImageAccountSelectionError("unavailable", f"no account available after {len(attempted_tokens)} attempts")
    ...
    def mark_image_result(self, access_token: str, success: bool) -> dict | None:
        ...
        with self._lock:
            ...
            if success:
                ...
                if not bool(next_item.get("image_quota_unknown")):
                    next_item["quota"] = max(0, int(next_item.get("quota") or 0) - 1)
                    if next_item["quota"] == 0:
                        next_item["status"] = "限流"
                        next_item["restore_at"] = next_item.get("restore_at") or None
                if next_item.get("status") == "限流" and bool(next_item.get("image_quota_unknown")):
                    next_item["status"] = "正常"
    ...
    def fetch_remote_info(
        ...
        self.update_account(access_token, {
            "last_remote_check_attempt_at": attempted_at,
        ...
        try:
            ...
            if refreshed_token and refreshed_token != active_token:
                ...
                self.update_account(active_token, {
                    "last_remote_check_error": str(exc)[:500],
                    "last_remote_check_result": "auth_invalid",
                }, quiet=True)
                raise
        self._record_refresh_success(active_token)
        checked_at = datetime.now(timezone.utc).isoformat()
        return self.update_account(active_token, {
            **result,
            "last_remote_checked_at": checked_at,
            "last_remote_check_attempt_at": checked_at,
    ...
    @classmethod
    def _pool_health_freshness_seconds(cls, value: int | float | None = None) -> int:
    ...
    @classmethod
    def _pool_metrics_from_accounts(cls, accounts: list[dict], freshness_seconds: int) -> dict[str, Any]:
        now = datetime.now(timezone.utc)
        normal = [item for item in accounts if cls._is_account_selectable(item, allow_limited=False)]
        confirmed = []
        for item in normal:
            checked_at = cls._parse_time(item.get("last_remote_checked_at"))
            if checked_at is not None and (now - checked_at).total_seconds() <= freshness_seconds:
                confirmed.append(item)
        return {
            "current_quota": sum(int(item.get("quota") or 0) for item in confirmed if not item.get("image_quota_unknown")),
            "current_available": len(confirmed),
            "estimated_quota": sum(int(item.get("quota") or 0) for item in normal if not item.get("image_quota_unknown")),
            "estimated_available": len(normal),
            "unconfirmed_available": max(0, len(normal) - len(confirmed)),
            "unknown_quota_count": sum(1 for item in confirmed if item.get("image_quota_unknown")),
    ...
    def evaluate_account_pool(
        ...
        *,
        ...
        metrics = self._pool_metrics_from_accounts(accounts, freshness)
        refreshed = 0
        errors: list[dict[str, Any]] = []
        target_reached = (
            target_quota is not None and metrics["current_quota"] >= max(1, int(target_quota))
        ) or (
            target_available is not None and metrics["current_available"] >= max(1, int(target_available))
        )
        if refresh_stale and not target_reached:
            now = datetime.now(timezone.utc)
            stale = []
            for item in accounts:
                ...
                if not token or item.get("status") != "正常":
                    continue
                checked_at = self._parse_time(item.get("last_remote_checked_at"))
                if checked_at is None or (now - checked_at).total_seconds() > freshness:
                    stale.append(token)
            if stale:
                result = self.refresh_accounts(stale[:10])
                refreshed = int(result.get("refreshed") or 0)
                errors = list(result.get("errors") or [])
                metrics = self._pool_metrics_from_accounts(self.list_accounts(), freshness)
        return {**metrics, "pool_refreshed": refreshed, "pool_refresh_errors": errors}
