...
INTERNAL_RESPONSE_KEYS = {
    "_account_email",
    "_conversation_id",
    "_image_upstream_attempts",
    "_image_upstream_selected",
}
...
class LoggedCall:
    ...
    async def run(self, handler, *args, sse: str = "openai"):
        ...
        if isinstance(result, dict):
            ...
            response.pop("_image_upstream_attempts", None)
            response.pop("_image_upstream_selected", None)
            return response
    ...
    def stream(self, items):
        ...
        image_upstream_attempts: list[dict[str, object]] | None = None
        image_upstream_selected = ""
        failed = False
        try:
            for item in items:
                if isinstance(item, dict):
                    attempts = item.get("_image_upstream_attempts")
                    if isinstance(attempts, list):
                        image_upstream_attempts = attempts
                    selected = item.get("_image_upstream_selected")
                    if selected:
                        image_upstream_selected = str(selected)
                urls.extend(_collect_urls(item))
            ...
            self.log(
                ...
                image_upstream_attempts=image_upstream_attempts or getattr(exc, "image_upstream_attempts", None),
                image_upstream_selected=image_upstream_selected,
            )
            ...
            if not failed:
                self.log("流式调用结束", urls=urls, account_email=account_emails[0] if account_emails else "",
                         ...
                         image_upstream_attempts=image_upstream_attempts,
                         image_upstream_selected=image_upstream_selected)

    def log(self, suffix: str, result: object = None, status: str = "success", error: str = "",
            urls: list[str] | None = None, account_email: str = "", conversation_id: str = "",
            image_upstream_attempts: object = None, image_upstream_selected: str = "") -> None:
        detail = {
        ...
        if isinstance(result, dict):
            attempts = result.get("_image_upstream_attempts")
            selected = result.get("_image_upstream_selected")
            if isinstance(attempts, list):
                detail["image_upstream_attempts"] = attempts
            if selected:
                detail["image_upstream_selected"] = str(selected)
        if isinstance(image_upstream_attempts, list):
            detail["image_upstream_attempts"] = image_upstream_attempts
        if image_upstream_selected:
            detail["image_upstream_selected"] = image_upstream_selected
        if error:
            detail["error"] = error
